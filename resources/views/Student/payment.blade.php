...
<body class="bg-gray-100 font-sans">
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="content-area flex-1 overflow-y-auto">
            ...
            <div class="p-6">
                ...
                @if($paymentDataToShow)
                    <div class="bg-white rounded-lg shadow-md overflow-hidden">
                        <div class="p-4 border-b border-gray-200">
                            <h3 class="text-lg font-semibold text-gray-800">Payment History</h3>
                        </div>
                        <div class="divide-y divide-gray-200">
                            @if(isset($paymentDataToShow['payment_records']) && $paymentDataToShow['payment_records']->count() > 0)
                                @foreach($paymentDataToShow['payment_records'] as $payment)
                                    <div class="p-4 hover:bg-gray-50 transition-colors">
                                        <div class="flex justify-between items-center">
                                            <div>
                                                <h4 class="font-medium text-gray-800">{{ $payment->program_name ?? 'Program Payment' }}</h4>
                                                <p class="text-sm text-gray-500 mt-1">Paid on: {{ \Carbon\Carbon::parse($payment->payment_date)->format('F j, Y') }}</p>
                                                @if($payment->transaction_id)
                                                    <p class="text-xs text-gray-400 mt-1">Transaction ID: {{ $payment->transaction_id }}</p>
                                                @endif
                                                @if($payment->or_number)
                                                    <p class="text-xs text-gray-400 mt-1">OR Number: {{ $payment->or_number }}</p>
                                                @endif
                                                <p class="text-xs text-gray-400">Payment Method: {{ ucfirst($payment->payment_method) }}</p>
                                            </div>
                                            <div class="text-right">
                                                <p class="font-semibold text-blue-600">₱{{ number_format($payment->amount, 2) }}</p>
                                                ...
                                                <button class="mt-2 text-blue-600 hover:text-blue-800 text-sm font-medium" onclick="viewReceipt({{ $payment->id }})">
                                                    View Receipt
                                                </button>
                                            </div>
                                        </div>
    ...
    <script>
        ...
        const paymentData = @json($paymentDataToShow['payment_records'] ?? []);
