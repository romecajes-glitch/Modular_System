                        ...
                        <div class="flex items-start space-x-4">
                            ...
                            <div class="flex-shrink-0">
                                @if($enrollment && $enrollment->photo)
                                    <img src="{{ asset('storage/' . $enrollment->photo) }}" alt="Profile" class="w-20 h-20 rounded-full object-cover border-4 border-white shadow-md">
                                @elseif($student && $student->photo)
                                    <img src="{{ asset('storage/' . $student->photo) }}" alt="Profile" class="w-20 h-20 rounded-full object-cover border-4 border-white shadow-md">
                                @else
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <p class="text-lg font-semibold text-gray-900">
                                            @if($enrollment)
                                                {{ $enrollment->first_name ?? '' }} {{ $enrollment->middle_name ?? '' }} {{ $enrollment->last_name ?? '' }} {{ $enrollment->suffix_name ?? '' }}
                                            @else
                                    ...
                                    <div>
                                        ...
                                        <div class="flex items-center space-x-2">
                                            <input type="email" id="emailInput" value="{{ $student->email ?? 'N/A' }}" class="flex-1 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" readonly>
                                    ...
                                    <div>
                                        ...
                                        <p class="text-gray-900">{{ $enrollment->phone ?? ($student->phone ?? 'N/A') }}</p>
                                    </div>
                                    
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">BirthDate:</label>
                                        <p class="text-gray-900">{{ $enrollment->birthdate ? \Carbon\Carbon::parse($enrollment->birthdate)->format('M d, Y') : 'N/A' }}</p>
                                    </div>
                                </div>
                    ...
                    <div>
                        ...
                        <ul class="space-y-3">
                            <li class="flex items-center">
                                ...
                                <span class="text-gray-600">Program: <span class="font-medium">{{ $enrollment->program->name ?? 'No Program' }}</span></span>
                            </li>
                            <li class="flex items-center">
                                <i class="fas fa-calendar-alt text-blue-500 mr-3 w-5"></i>
                                <span class="text-gray-600">Enrolled: <span class="font-medium">{{ $enrollment->created_at->format('M d, Y') ?? 'N/A' }}</span></span>
                            </li>
                            <li class="flex items-center">
                                <i class="fas fa-info-circle text-blue-500 mr-3 w-5"></i>
                                <span class="text-gray-600">Status: <span class="font-medium">{{ $enrollment->status ?? 'N/A' }}</span></span>
                            </li>
                        </ul>
                    </div>
                    ...
                    <div>
                        ...
                        <ul class="space-y-3">
                            <li class="flex items-center">
                                ...
                                <span class="text-gray-600">Attendance: <span class="font-medium">{{ $enrollment->attendance_percentage ?? 0 }}%</span></span>
                            </li>
                            <li class="flex items-center">
                                <i class="fas fa-money-bill-wave text-blue-500 mr-3 w-5"></i>
                                <span class="text-gray-600">Payment Status: <span class="font-medium">{{ $enrollment->payment_status ?? 'Pending' }}</span></span>
                            </li>
                            <li class="flex items-center">
                                <i class="fas fa-certificate text-blue-500 mr-3 w-5"></i>
                                <span class="text-gray-600">Certificate: <span class="font-medium">{{ $enrollment->isEligibleForCertificate() ? 'Eligible' : 'Not Eligible' }}</span></span>
                            </li>
                        </ul>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <p class="text-gray-900">{{ $enrollment->age ?? 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Gender:</label>
                            <p class="text-gray-900">{{ $enrollment->gender ?? 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Civil Status:</label>
                            <p class="text-gray-900">{{ $enrollment->civil_status ?? 'N/A' }}</p>
                        </div>
                        @if($enrollment->civil_status == 'Married')
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Spouse Name:</label>
                            <p class="text-gray-900">{{ $enrollment->spouse_name ?? 'N/A' }}</p>
                        </div>
                        @endif
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Citizenship:</label>
                            <p class="text-gray-900">{{ $enrollment->citizenship ?? 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Religion:</label>
                            <p class="text-gray-900">{{ $enrollment->religion ?? 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Place of Birth:</label>
                            <p class="text-gray-900">{{ $enrollment->place_of_birth ?? 'N/A' }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <p class="text-gray-900">{{ $enrollment->created_at->format('M d, Y') ?? 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Status:</label>
                            <p class="text-gray-900">{{ $enrollment->status ?? 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Registration Fee:</label>
                            <p class="text-gray-900">₱{{ number_format($enrollment->program->registration_fee ?? 0, 2) }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <p class="text-gray-900">{{ $enrollment->attendance_percentage ?? 0 }}%</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Payment Status:</label>
                            <p class="text-gray-900">{{ $enrollment->payment_status ?? 'Pending' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Certificate Eligibility:</label>
                            <p class="text-gray-900">{{ $enrollment->isEligibleForCertificate() ? 'Eligible' : 'Not Eligible' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Last Updated:</label>
                            <p class="text-gray-900">{{ $enrollment->updated_at->format('M d, Y H:i') ?? 'N/A' }}</p>
                        </div>
                    </div>
                </div>
            </div>
...
function saveEmail() {
    ...
    fetch('{{ route("student.send-email-otp") }}', {
        method: 'POST',
        headers: {
...
document.getElementById('changePasswordForm').addEventListener('submit', function(e) {
    ...
    fetch('{{ route("student.change-password") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
