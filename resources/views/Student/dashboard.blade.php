...
<body class="bg-gray-100 font-sans">
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="content-area flex-1 overflow-y-auto content-scrollbar">
                    ...
                    <div class="relative group">
                        <button id="adminDropdown" class="flex items-center focus:outline-none space-x-3 bg-gradient-to-r from-blue-500 to-purple-600 text-white px-4 py-2 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300">
                            <div class="h-8 w-8 rounded-full bg-white bg-opacity-20 flex items-center justify-center">
                                @if($enrollment && $enrollment->photo)
                                    <img src="{{ asset('storage/' . $enrollment->photo) }}" alt="Profile" class="w-8 h-8 rounded-full object-cover">
                                @elseif($student && $student->photo)
                                    <img src="{{ asset('storage/' . $student->photo) }}" alt="Profile" class="w-8 h-8 rounded-full object-cover">
                                @else
            ...
            <div class="p-6">
                ...
                @if($enrollmentStatusMessage && $enrollmentStatusMessage['type'] !== 'success')
                    ...
                    <div id="enrollmentNotificationModal" class="fixed inset-0 flex items-center justify-center z-50 p-4" style="background: rgba(0, 0, 0, 0.3); backdrop-filter: blur(4px);">
                        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md mx-auto transform transition-all duration-500 scale-100 max-h-[90vh] overflow-y-auto modal-scrollbar" style="box-shadow: 0 32px 64px -12px rgba(0, 0, 0, 0.25);">
                            ...
                            <div class="relative overflow-hidden rounded-t-3xl">
                                @if($enrollmentStatusMessage['type'] === 'error')
                                ...
                                @elseif($enrollmentStatusMessage['type'] === 'warning')
                            ...
                            <div class="p-8">
                                ...
                                <div class="space-y-4">
                                    @if($enrollmentStatusMessage['type'] === 'error' && isset($enrollmentStatusMessage['show_payment_button']) && $enrollmentStatusMessage['show_payment_button'])
                                        <button id="payRegistrationFeeBtn" onclick="payRegistrationFee()" class="w-full bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white font-bold py-4 px-6 rounded-2xl transition-all duration-300 flex items-center justify-center shadow-lg hover:shadow-xl transform hover:-translate-y-1">
                                            ...
                                            <span class="text-lg">Pay Registration Fee (₱{{ number_format($enrollmentStatusMessage['registration_fee'] ?? 0, 2) }})</span>
                                        </button>
                                    @elseif($enrollmentStatusMessage['type'] === 'error' && isset($enrollmentStatusMessage['show_reapply_button']) && $enrollmentStatusMessage['show_reapply_button'])
                                        <a href="{{ route('enrollment.reapply', $enrollment->id) }}" 
                ...
                <div class="bg-gradient-to-r from-blue-500 to-blue-700 rounded-xl p-6 text-white mb-6 shadow-lg">
                    <h2 class="text-2xl font-bold mb-2">Welcome back, {{ ucfirst(explode(' ', auth()->user()->name)[0]) }}!</h2>
                ...
                <div class="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-xl shadow-lg p-6 mb-6 text-white">
                    <div class="flex items-center justify-between">
                        <div>
                            <h3 class="text-lg font-semibold mb-1">{{ $enrollment->program->name ?? 'No Program Enrolled' }}</h3>
                            <p class="text-indigo-100">{{ $enrollment->program->description ?? 'No Program Description Available' }}</p>
                        </div>
                    ...
                    <div class="mt-4 pt-4 border-t border-white/20">
                        ...
                        <div>
                            ...
                            <p class="font-medium">
                                @if(isset($expectedGraduation))
                                    {{ \Carbon\Carbon::parse($expectedGraduation)->format('M d, Y') }}
                                @else
                                    N/A
                                @endif
                            </p>
                        ...
                        <div class="mt-4">
                            ...
                            <div class="w-full bg-white/20 rounded-full h-2">
                                <div class="bg-white h-2 rounded-full" style="width: {{ $progressPercentage ?? 0 }}%"></div>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="flex items-center">
                                ...
                                <div>
                                    ...
                                    <h3 class="text-2xl font-bold text-gray-800">
                                        {{ $attendance['attended_sessions'] ?? '--' }}/{{ $attendance['total_sessions'] ?? '--' }}
                                    </h3>
                                </div>
                            </div>
                            <span class="text-sm font-medium text-blue-600">{{ isset($attendance['attendance_percentage']) ? $attendance['attendance_percentage'] . '%' : '--' }}</span>
                        </div>
                        <div class="mt-4">
                            <div class="flex justify-between text-xs text-gray-500 mb-1">
                            <span>Attended: {{ $attendance['attended_sessions'] ?? '--' }} sessions</span>
                            <span>Remaining: {{ isset($attendance['total_sessions'], $attendance['attended_sessions']) ? $attendance['total_sessions'] - $attendance['attended_sessions'] : '--' }} sessions</span>
                            </div>
                            <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="h-2 bg-gradient-to-r from-blue-400 to-blue-600 rounded-full" style="width: {{ $attendance['attendance_percentage'] ?? 0 }}%"></div>
                            </div>
                            <div class="mt-2 flex justify-between text-xs">
                            <span class="text-gray-500">Last attended: {{ $attendance['last_attended'] ?? '--' }}</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center">
                                ...
                                <div>
                                    ...
                                    <h3 class="text-xl font-bold">${{ number_format($payments['total_paid'] ?? 0) }} Paid</h3>
                                </div>
                            </div>
                            <span class="text-xs bg-green-100 text-green-800 px-2 py-1 rounded-full capitalize">{{ $payments['payment_status'] ?? 'current' }}</span>
                        </div>
                        <div class="mt-4 space-y-2">
                            <div class="flex justify-between text-sm">
                                <span class="text-gray-600">Latest Payment Date:</span>
                                <span class="font-medium">{{ $payments['payment_date'] ?? '--' }}</span>
                            </div>
                            <div class="flex justify-between text-sm">
                                <span class="text-gray-600">Amount Paid:</span>
                                <span class="font-medium">${{ number_format($payments['total_paid'] ?? 0) }}</span>
                            </div>
                            <div class="flex justify-between text-sm">
                                <span class="text-gray-600">Payment Method:</span>
                                <span class="font-medium">{{ $payments['payment_method'] ?? '--' }}</span>
                            </div>
                        </div>
                            <button onclick="window.location.href='{{ route('student.payment') }}'" class="mt-3 w-full text-sm bg-green-50 hover:bg-green-100 text-green-600 py-1 px-3 rounded-lg transition duration-200 cursor-pointer">
                    ...
                    <div class="bg-white rounded-lg shadow p-6">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center">
                                ...
                                <div>
                                    ...
                                    <h3 class="text-xl font-bold">{{ $certificates['total_certificates'] ?? 0 }} Earned</h3>
                                </div>
                            </div>
                            @if($certificates['is_eligible'] ?? false)
                        ...
                        <div class="mt-4">
                            @if($certificates['is_eligible'] ?? false)
                                ...
                                <p class="text-xs text-gray-500 mt-1">Attendance: {{ $certificates['attendance_percentage'] ?? 0 }}%</p>
                            @else
                                <p class="text-sm text-gray-600">
                                    @if(($certificates['attendance_percentage'] ?? 0) > 0)
                                        {{ 80 - ($certificates['attendance_percentage'] ?? 0) }}% more attendance needed
                                    @else
                                        Not yet eligible
                                    @endif
                                </p>
                                <p class="text-xs text-gray-500 mt-1">Attendance: {{ $certificates['attendance_percentage'] ?? 0 }}% / 100% required</p>
                            @endif
                            <button onclick="window.location.href='{{ route('student.certificate') }}'" class="mt-2 text-sm text-purple-600 hover:text-purple-800 font-medium cursor-pointer">View Certificates</button>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-lg shadow p-6">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center">
                                ...
                                <div>
                                    ...
                                    <h3 class="text-xl font-bold">{{ $thisWeekSessionsCount }} This Week</h3>
                                </div>
                            </div>
                        ...
                        <div class="mt-4 space-y-2">
                            @if(count($upcomingSessions) > 0)
                                ...
                                @endforeach
                                @if(count($upcomingSessions) > 2)
                                    <div class="text-xs text-gray-500 mt-2">
                                        +{{ count($upcomingSessions) - 2 }} more sessions
                                    </div>
                                @endif
                            @else
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div class="bg-white rounded-lg shadow p-6">
                        ...
                        <button onclick="window.location.href='{{ route('student.attendance') }}'" class="w-full bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded-lg transition duration-200 cursor-pointer">
                            View Attendance
                        </button>
                    </div>
                    <div class="bg-white rounded-lg shadow p-6">
                        ...
                        <button onclick="window.location.href='{{ route('student.payment') }}'" class="w-full bg-green-600 hover:bg-green-700 text-white py-2 px-4 rounded-lg transition duration-200 cursor-pointer">
                            View Payments
                        </button>
                    </div>
                    <div class="bg-white rounded-lg shadow p-6">
                        ...
                        <button onclick="window.location.href='{{ route('student.certificate') }}'" class="w-full bg-purple-600 hover:bg-purple-700 text-white py-2 px-4 rounded-lg transition duration-200 cursor-pointer">
                            View Certificates
                        </button>
                    </div>
                </div>
    ...
    <script>
        ...
        function initiatePayMongoCheckout() {
            ...
            const amount = {{ $registrationFeeAmount }};
            const email = '{{ auth()->user()->email }}';
            const sessionCount = 1; // Registration fee is for 1 session

            fetch('{{ route("payment.direct") }}', {
                method: 'POST',
                headers: {
