...
<body class="bg-gray-100 font-sans">
        ...
        <div class="sidebar bg-blue-900 text-white w-56 flex flex-col">
            ...
            <div class="user-profile p-4 flex items-center border-b border-blue-800">
                ...
                <div class="ml-3 user-details">
                    ...
                    <div class="text-xs text-blue-200">{{ $program->name ?? 'No Program Enrolled' }}</div>
                </div>
            </div>
            ...
            <div class="p-6">
                ...
                <div class="bg-white rounded-lg shadow-md p-6">
                    <div class="flex items-center justify-between mb-6">
                        <h3 class="text-lg font-semibold text-gray-800 mb-4">{{ $program->name ?? 'No Program' }}</h3>
                        @php
                            $isDisabled = in_array($enrollment->status, ['pending', 'rejected']);
                        @endphp
                        <button onclick="openPaymentModal()" 
                                class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center justify-center gap-2 transition-colors {{ $isDisabled ? 'opacity-50 cursor-not-allowed' : '' }}"
                                {{ $isDisabled ? 'disabled' : '' }}>
                                ...
                                @if($paidSessions > 0)
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            @if($amountPaid)
                                                <span class="font-medium text-blue-600">₱{{ number_format($amountPaid, 2) }}</span>
                                            @else
                                    ...
                                    @if($paidSessions < $totalSessions)
                                        @for($i = $paidSessions + 1; $i <= $totalSessions; $i++)
                    ...
                    <script>
                            ...
                            for (let i = 1; i <= paidSessions; i++) {
                                const row = `
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $instructors->implode(', ') ?: 'N/A' }}</td>
                                    </tr>
                                `;
                        ...
                        function fetchUpdatedAttendance() {
                            fetch('{{ route("student.attendance.ajax") }}', {
                                method: 'GET',
                                headers: {
            ...
            <div class="p-6">
                ...
                <div class="bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl p-4 mb-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <h4 class="font-semibold text-gray-800">{{ $program->name ?? 'No Program' }}</h4>
                            <p class="text-sm text-gray-600">Price per Session</p>
                        </div>
                        <div class="text-right">
                            <div class="text-2xl font-bold text-blue-600">₱{{ number_format($program->price_per_session ?? 500, 2) }}</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="relative">
                        <select id="sessionCount" 
                                class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 font-medium {{ $allSessionsCompleted ? 'bg-gray-100 text-gray-500 cursor-not-allowed' : 'bg-white text-gray-700' }}"
                                {{ $allSessionsCompleted ? 'disabled' : '' }}>
                            @php
                                $totalSessions = $attendance['total_sessions'] ?? 20;
                            ...
                            @if($paidSessions < $totalSessions)
                                @for($i = 1; $i <= ($totalSessions - $paidSessions); $i++)
                                    <option value="{{ $i }}">{{ $i }} Session{{ $i > 1 ? 's' : '' }} (Session{{ $i == 1 ? '' : 's' }} {{ $nextSessionToPay }}{{ $i > 1 ? '-' . ($nextSessionToPay + $i - 1) : '' }})</option>
                                @endfor
                            @else
                        ...
                        <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                            <i class="fas {{ $allSessionsCompleted ? 'fa-check-circle text-green-500' : 'fa-chevron-down text-gray-400' }}"></i>
                        </div>
                    </div>
                ...
                <!-- Total Amount -->
                <div class="bg-gradient-to-r {{ $allSessionsCompleted ? 'from-gray-50 to-gray-100 border-2 border-gray-200' : 'from-green-50 to-green-100 border-2 border-green-200' }} rounded-xl p-4 mb-6">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            <div class="{{ $allSessionsCompleted ? 'bg-gray-500' : 'bg-green-500' }} p-2 rounded-full mr-3">
                                <i class="fas {{ $allSessionsCompleted ? 'fa-check-circle' : 'fa-calculator' }} text-white"></i>
                            </div>
                            <div>
                                <p class="text-sm font-medium {{ $allSessionsCompleted ? 'text-gray-800' : 'text-green-800' }}">
                                    {{ $allSessionsCompleted ? 'Program Completed' : 'Total Amount' }}
                                </p>
                                <p class="text-xs {{ $allSessionsCompleted ? 'text-gray-600' : 'text-green-600' }}">
                                    {{ $allSessionsCompleted ? 'All sessions have been paid for' : 'Including all selected sessions' }}
                                </p>
                            </div>
                        </div>
                        <div class="text-right">
                            ...
                            @else
                                <div id="totalAmount" class="text-2xl font-bold text-green-600">₱{{ number_format($program->price_per_session ?? 500, 2) }}</div>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <!-- Payment Form -->
                <form id="paymentForm" action="{{ route('payment.checkout') }}" method="POST">
                    @csrf
                    <input type="hidden" name="amount" id="paymentAmount" value="{{ $program->price_per_session ?? '500' }}">
                    ...
                    <button type="button" 
                            ...
                            id="paymentButton"
                            class="w-full font-semibold py-4 px-6 rounded-xl transition-all duration-300 flex items-center justify-center shadow-lg mb-3 {{ $allSessionsCompleted ? 'bg-gray-400 text-gray-600 cursor-not-allowed opacity-60' : 'bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white hover:shadow-xl transform hover:-translate-y-1' }}"
                            {{ $allSessionsCompleted ? 'disabled' : '' }}>
                        <i class="fas {{ $allSessionsCompleted ? 'fa-check-circle' : 'fa-wallet' }} mr-3 text-lg"></i>
                        <span class="text-lg">
                            {{ $allSessionsCompleted ? 'All Sessions Completed' : 'Proceed to Payment via GCash' }}
                        </span>
                    </button>
                </form>
    ...
    <script>
        ...
        function updateTotalAmount() {
            ...
            const pricePerSession = {{ $program->price_per_session ?? 500 }};
            const paidSessions = {{ $paidSessions ?? 0 }};
