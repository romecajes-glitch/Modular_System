            ...
            <div class="flex flex-col md:flex-row items-center justify-between">
                <div class="flex flex-1 items-center">
                    <div class="relative group mr-6" title="Click to change profile picture">
                        @if(Auth::user()->photo)
                            <img src="{{ asset('storage/' . Auth::user()->photo) }}" alt="Profile" class="w-24 h-24 rounded-full border-4 border-white shadow-md object-cover hover:opacity-80 transition-all duration-300 hover:scale-105">
                        @else
                    ...
                    <div class="flex flex-col space-y-2">
                        <div class="relative">
                            <h2 class="text-2xl font-bold text-white" style="text-shadow: 0 0 20px rgba(30,58,138,0.8), 0 0 40px rgba(30,58,138,0.6), 0 0 60px rgba(30,58,138,0.4); filter: drop-shadow(0 0 10px rgba(30,58,138,0.5));">{{ Auth::user()->name }}</h2>
                        </div>
                        <div class="relative">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <div id="email-edit" class="hidden flex items-center">
                            <input type="email" id="profile-email-input" value="{{ Auth::user()->email ?? '' }}" class="flex-1 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <div class="space-y-2">
                        ...
                        <div id="phone-edit" class="hidden flex items-center">
                            <input type="tel" id="profile-phone-input" value="{{ Auth::user()->phone ?? '' }}" maxlength="11" pattern="09[0-9]{9}" placeholder="09XXXXXXXXX" class="flex-1 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" oninput="validatePhoneInput(this)" onkeypress="return isNumberKey(event)">
...
function sendEmailOTP(email) {
    ...
    fetch('{{ route("admin.email.send_otp") }}', {
        method: 'POST',
        body: formData,
        headers: {
...
function verifyEmailOTP(email) {
    ...
    fetch('{{ route("admin.email.verify_otp") }}', {
        method: 'POST',
        body: formData,
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
...
function updateProfileField(field, value, displayElement) {
    ...
    switch(field) {
        case 'phone':
            route = '{{ route("admin.phone.update") }}';
            data.phone = value;
            break;
        case 'gender':
            route = '{{ route("admin.gender.update") }}';
            data.gender = value;
            break;
        case 'birthdate':
            route = '{{ route("admin.birthdate.update") }}';
...
function uploadProfilePicture(input) {
    ...
    if (input.files && input.files[0]) {
        ...
        // Upload the file
        console.log('Uploading to:', '{{ route("admin.profile.photo.update") }}');
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                if (profileImg) {
                    const newSrc = '{{ asset("storage/") }}/' + data.path + '?t=' + new Date().getTime();
